module BBLib::Attr
  ...
  def attr_valid_file(*methods, raise: true, **opts)
    methods.each { |m| attr_type(m, opts, &attr_set(m, opts) { |x| File.exist?(x.to_s) ? x.to_s : (raise ? raise(ArgumentError, "File '#{x}' does not exist. @#{m} must be set to a valid file location!") : nil) }) }
  end

  def attr_valid_dir(*methods, raise: true, **opts)
    methods.each { |m| attr_type(m, opts, &attr_set(m, opts) { |x| Dir.exist?(x.to_s) ? x.to_s : (raise ? raise(ArgumentError, "Dir '#{x}' does not exist. @#{m} must be set to a valid directory location!") : nil) }) }
  end
