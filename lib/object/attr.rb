...
class Class
  ...
  def attr_type method, &block
    define_method("#{method}=", &block)
    define_method(method){ instance_variable_get("@#{method}")}
  end

  def attr_sender call, *methods, **opts
    methods.each{ |m| attr_type(m, &attr_set(m, opts.merge(sender: true)){ |x| x.send(call) } )}
  end

  def attr_of klass, *methods, **opts
    methods.each{ |m| attr_type(m, &attr_set(m, opts){ |x|
        if x.is_a?(klass)
  ...
  def attr_boolean *methods, no_q: false
    methods.each{ |m|
      attr_type(m) { |x| instance_variable_set("@#{m}", !!x && x.to_s != 'false') }
      alias_method "#{m}?", m unless no_q
    }
  end
  ...
  def attr_integer_between min, max, *methods, **opts
    methods.each{ |m| attr_type(m, &attr_set(m, opts){ |x| BBLib::keep_between(x, min, max) })}
  end
  ...
  def attr_symbol *methods, **opts
    methods.each{ |m| attr_type(m, &attr_set(m, opts){ |x| x.to_s.to_sym } )}
  end

  alias_method :attr_sym, :attr_symbol

  def attr_clean_symbol *methods, **opts
    methods.each{ |m| attr_type(m, &attr_set(m, opts){ |x| x.to_s.to_clean_sym } )}
  end

  alias_method :attr_clean_sym, :attr_clean_symbol

  def attr_array *methods, **opts
    methods.each{ |m| attr_type(m, &attr_set(m, opts){ |*x| instance_variable_set("@#{m}", x) } )}
  end
  ...
  def attr_element_of list, *methods, **opts
    methods.each do |m|
      attr_type(m, &attr_set(m, opts) do |x|
          if !list.include?(x)
  ...
  def attr_hash *methods, **opts
    methods.each{ |m| attr_type(m, &attr_set(m, opts) do |*a, **x|
          begin
  ...
  def attr_valid_file *methods, raise: true, **opts
    methods.each{ |m| attr_type(m, &attr_set(m, opts){ |x| File.exists?(x.to_s) ? x.to_s : (raise ? raise(ArgumentError, "File '#{x}' does not exist. @#{m} must be set to a valid file location!") : nil)} )}
  end

  def attr_valid_dir *methods, raise: true, **opts
    methods.each{ |m| attr_type(m, &attr_set(m, opts){ |x| Dir.exists?(x.to_s) ? x.to_s : (raise ? raise(ArgumentError, "Dir '#{x}' does not exist. @#{m} must be set to a valid directory location!") : nil)} )}
  end

  def attr_set method, allow_nil: false, fallback: :_nil, sender: false, &block
    proc{ |x|
