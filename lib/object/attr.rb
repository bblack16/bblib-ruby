module BBLib
  ...
  module Attr
    ...
    def attr_type method, opts, &block
      ...
      instance_variable_set("@#{method}", opts[:default])
    end

    def attr_sender call, *methods, **opts
      methods.each{ |m| attr_type(m, opts, &attr_set(m, opts.merge(sender: true)){ |x| x.send(call) } )}
    end
    ...
    def attr_hash *methods, **opts
      methods.each{ |m| attr_type(m, &attr_set(m, opts) do |*a|
            begin
